...
const TEXT_TOOLS = [
  { href: "/text/font-generator", key: "fontGenerator" },
  { href: "/text/fb-post-formatter", key: "fbPostFormatter" },
] as const;

export default function TextToolsNav() {
  const t = useTranslations("TextToolsNav");
  const pathname = usePathname();

  return (
    <div className="mx-auto max-w-7xl px-6 2xl:max-w-[1600px]">
      <nav className="flex gap-0 border-b border-border">
        {TEXT_TOOLS.map(({ href, key }) => {
